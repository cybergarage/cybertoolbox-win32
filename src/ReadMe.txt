========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : CyberToolBox
========================================================================


AppWizard が生成した CyberToolBox には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには CyberToolBox を構成している各ファイルの概要説明が含まれています。


CyberToolBox.h
    このファイルはアプリケーションの中心となるインクルードファイルです。この
    ファイルは他のプロジェクトの固有のインクルードファイル（Resource.h  も含
    みます） をインクルードし、また CCyberToolBoxApp アプリケーションクラスを宣
    言します。

CyberToolBox.cpp
    このファイルは CCyberToolBoxApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\CyberToolBox.ico
    ...
    ルです。このアイコンはリソース ファイル CyberToolBox.rc によってインクルード
	されます。
...
CyberToolBoxDoc.h, CyberToolBoxDoc.cpp - ドキュメント
    これらのファイルは CCyberToolBoxDoc クラスを含みます。特別なドキュメント デ
    ータを付け加えたり、CCyberToolBoxDoc::Serialize を使ったファイルのセーブや
    ロード機能をインプリメントしたいときはこのファイルを編集してください。

CyberToolBoxView.h, CyberToolBoxView.cpp - ビュー
    これらのファイルは CCyberToolBoxView クラスを含みます。
    CCyberToolBoxView オブジェクトは CCyberToolBoxDoc オブジェクトを表示するために
    使います。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) CyberToolBox.pch 
