...
*
*	File:	FilterPosition2DInterpolator.java
----------------------------------------------------------------*/
...
public class FilterPosition2DInterpolator extends Script {
    ...
    public void initialize () {
		...
		if (index != -1) {
			try {
				...
				int indexCanma = string.indexOf(',');
				if (indexCanma != -1) {
					value = new Double(new String(string.toCharArray(), 0, (indexCanma-1)+1));
					range[0][0] = value.doubleValue();
					value = new Double(new String(string.toCharArray(), indexCanma+1, (string.length()-1) - indexCanma));
		...
		if (bExecution) {
			if (range[0][0] != range[1][0] || range[0][1] != range[1][1]) {
				try {
					...
					outValue.setValue(keyValue[0] + "," + keyValue[1]);
				}
				catch (NumberFormatException exp) {
					outValue.setValue("");
				}
